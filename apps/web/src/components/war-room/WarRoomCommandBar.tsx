...
export function WarRoomCommandBar({ workflow }: { workflow: WorkflowViewModel }) {
  ...
  const decisionTone = workflow.approvalState === "approved" ? "success" : workflow.approvalState === "pending" ? "warning" : "neutral";

  return (
    <section className="relay-command-bar" aria-label="War room command summary">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <Badge tone="accent">Live demo console</Badge>
          <Badge>{workflow.case.id}</Badge>
          <Badge tone={decisionTone}>Approval: {workflow.approvalState}</Badge>
        </div>
        <h1 className="mt-3 truncate text-xl font-bold tracking-tight md:text-2xl">Sentinel Relay War Room</h1>
        <p className="mt-1 max-w-4xl text-sm leading-6 text-slate-400">
          A judge-facing command surface showing agents coordinating through a shared Band-style incident room.
        </p>
      </div>
      ...
      <dl className="grid min-w-full grid-cols-2 gap-2 sm:min-w-[460px] sm:grid-cols-4">
        ...
        <div className="relay-mini-stat">
          <dt>Agents</dt>
          <dd>{activeAgents}/{workflow.agents.length}</dd>
        </div>
        <div className="relay-mini-stat">
          <dt>Messages</dt>
          <dd>{workflow.messages.length}/{workflow.totalSteps}</dd>
        </div>
      </dl>
    </section>
  );
