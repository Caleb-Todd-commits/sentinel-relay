    ...
    <section className="relay-card relay-approval-card" aria-labelledby="approval-gate-heading">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 id="approval-gate-heading" className="mt-3 text-xl font-semibold">Containment approval required</h2>
          <p className="mt-2 text-sm leading-6 text-slate-300">The workflow intentionally pauses before high-impact production action.</p>
        </div>
        <Badge tone={approved ? "success" : "warning"}>{approved ? "approved" : "pending"}</Badge>
      </div>
      <p className="mt-4 rounded-2xl border border-slate-700/80 bg-slate-950/30 p-4 text-sm leading-6 text-slate-200">{request.action}</p>
      ...
      {approved && decision ? (
        <div className="mt-4 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 p-4">
          <p className="text-sm font-semibold text-emerald-100">{decision.decidedBy}: approved with scoped limits</p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4">
          <p className="flex-1 text-sm leading-6 text-slate-300">Remediation stays blocked until this decision is recorded. This is the safety gate that keeps the demo enterprise-ready.</p>
          {onApprove ? <button type="button" onClick={onApprove} className="relay-button-primary">Approve containment</button> : null}
        </div>
      )}
    </section>
  );
